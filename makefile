...
deploy-publisher: build-publisher ## builds and deploy the publisher service on Minishift
	eval $$(minishift docker-env) && docker login -u developer -p $(shell oc whoami -t) $(shell minishift openshift registry) && docker tag fabric8-publisher:latest $(MINISHIFT_REGISTRY)/$(MINISHIFT_PROJECT)/publisher:latest
	eval $$(minishift docker-env) && docker login -u developer -p $(shell oc whoami -t) $(shell minishift openshift registry) && docker push $(MINISHIFT_REGISTRY)/$(MINISHIFT_PROJECT)/publisher:latest
...
deploy-subscribers: build-subscriber ## builds and deploy the subscriber service 1 on Minishift
	eval $$(minishift docker-env) && docker login -u developer -p $(shell oc whoami -t) $(shell minishift openshift registry) && docker tag fabric8-subscriber:latest $(MINISHIFT_REGISTRY)/$(MINISHIFT_PROJECT)/subscriber:latest
	eval $$(minishift docker-env) && docker login -u developer -p $(shell oc whoami -t) $(shell minishift openshift registry) && docker push $(MINISHIFT_REGISTRY)/$(MINISHIFT_PROJECT)/subscriber:latest
